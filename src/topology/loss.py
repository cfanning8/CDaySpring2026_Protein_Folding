...
def topological_loss_batch_torch(vpd_diffs: torch.Tensor, loss_fn: TopologicalRKHSLoss) -> torch.Tensor:
    """
    Reference-compatible batch mean; **not differentiable** — each row is evaluated via numpy
    inside `TopologicalRKHSLoss`. Do not use for end-to-end backprop until a torch-native RFF path exists.
    """
