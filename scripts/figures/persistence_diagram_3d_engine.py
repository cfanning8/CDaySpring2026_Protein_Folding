...
def _diagonal_birth_death_t(
    xlim: Tuple[float, float], ylim: Tuple[float, float]
) -> np.ndarray | None:
    """
    Points (t, t) on the birth=death line that lie inside the viewing rectangle
    [xlim[0], xlim[1]] × [ylim[0], ylim[1]]. Using linspace(xlim[0], min(xlim[1], ylim[1]))
    is wrong when ylim[0] > xlim[0] (diagonal shifts off the true y=x segment).
    """
    t0 = max(float(xlim[0]), float(ylim[0]))
    t1 = min(float(xlim[1]), float(ylim[1]))
    if not (np.isfinite(t0) and np.isfinite(t1)) or t1 <= t0:
        return None
    return np.linspace(t0, t1, 128)
...
def render_persistence_diagram_3d(
    ...
    # 3. Diagonal on the floor: birth = death, clipped to the axis box
    diag = _diagonal_birth_death_t(xlim, ylim)
    if diag is not None:
...
def render_virtual_persistence_diagram_3d(
    ...
    if len(diff) == 0:
        ...
        ax.set_zlabel("Multiplicity", fontsize=12, labelpad=10)
        for val in np.linspace(xlim[0], xlim[1], 6):
            ax.plot([val, val], [ylim[0], ylim[1]], [0, 0], color=COLOR_FLOOR_GRID, linewidth=2, alpha=0.6)
        for val in np.linspace(ylim[0], ylim[1], 6):
            ax.plot([xlim[0], xlim[1]], [val, val], [0, 0], color=COLOR_FLOOR_GRID, linewidth=2, alpha=0.6)
        xx, yy = np.meshgrid(
            np.linspace(xlim[0], xlim[1], 2),
            np.linspace(ylim[0], ylim[1], 2),
        )
    ...
    # 3. Diagonal (birth = death, clipped to axis box)
    diag = _diagonal_birth_death_t(xlim, ylim)
    if diag is not None:
